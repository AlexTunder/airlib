...
NetworkListener.setStopCom(char[3]) - set 3 byte for stop. If in request have sub-string with this bytes, listening will be stopped
NetworkListener.setStreamCom(char[3]) - set go-to-stream command. It's need for big bitrade and archive algorithms (not realised)
...
Tips:
    ...
    try to use only DEFAULT_LISTENER as type and use only this type. Other types in development (have issiue about byte-command interface)
